...
GENRES_LIST = (requests.get(f"{BASE_URL}genre/movie/list",params={"api_key": os.getenv('API_KEY')})).json()['genres']
...
def show_trending_paged(page):
    ...
    trending = requests.get(f"{BASE_URL}trending/movie/week",params={"api_key": os.getenv('API_KEY'), "page": page})
...
def show_popular_paged(page):
    ...
    popular = requests.get(f"{BASE_URL}movie/popular",params={"api_key": os.getenv('API_KEY'), "page": page})
    # rendering a template and passing the json version of the results, also passing the genres and the page number
...
def show_top_paged(page):
    ...
    # getting the list of movies and mapping into a variable. Passing a page number from the url for use in pagination.
    top = requests.get(f"{BASE_URL}movie/top_rated",params={"api_key": os.getenv('API_KEY'), "page": page})
    # rendering a template and passing the json version of the results, also passing the genres and the page number
...
def show_movie_details(id):
    ...
    movie = requests.get(f"{BASE_URL}movie/{id}",params={"api_key": os.getenv('API_KEY')})
...
def show_movie_images(id):
    ...
    images = requests.get(f"{BASE_URL}movie/{id}/images",params={"api_key": os.getenv('API_KEY'), "language": "en"})
...
def show_playlist_watched():
    ...
    for id in watched_movie_ids:
        ...
        watched_movies.append(requests.get(f"{BASE_URL}movie/{id}",params={"api_key": os.getenv('API_KEY')}).json())
...
def show_private_playlist_details(id):
    ...
    for m_id in p_movie_ids:
        ...
        p_movies.append(requests.get(f"{BASE_URL}movie/{m_id}",params={"api_key": os.getenv('API_KEY')}).json())
...
def show_search_results(q, page):
    ...
    results = requests.get(f"{BASE_URL}search/movie",params={"api_key": os.getenv('API_KEY'), "query": q, "page": page})
